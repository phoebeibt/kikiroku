function Dot({ filled, size, emptyColor }) {
  if (filled) return (
    <svg width={size} height={size} viewBox="0 0 44 44" style={{ display: 'inline-block', verticalAlign: 'middle' }}>
      <g transform="translate(22,22)">
        <path d="M-3,-9 C-7,-7 -10,-3 -10,2 C-10,7 -7,10 -2,11 C3,12 8,9 10,5 C11,2 10,-3 7,-6 C4,-8 0,-9 -2,-9"
          fill="none" stroke="#7C3A28" strokeOpacity=".5" strokeWidth="2.8" strokeLinecap="round" />
        <circle r="5" fill="#4A7A35" />
      </g>
    </svg>
  )
  return (
    <svg width={size} height={size} viewBox="0 0 44 44" style={{ display: 'inline-block', verticalAlign: 'middle' }}>
      <g transform="translate(22,22)">
        <circle r="5" fill="none" stroke={emptyColor} strokeWidth="2.2" opacity="0.3" />
      </g>
    </svg>
  )
}
  ...
  return (
    <span style={{ display: 'flex', alignItems: 'center', gap: 3 }}>
      {Array.from({ length: max }, (_, i) => (
        <Dot key={i} filled={i < r} size={size} emptyColor="#7C3A28" />
      ))}
    </span>
  )
}
...
export function StarsLight({ rating, max = 5, size = 11 }) {
  ...
  return (
    <span style={{ display: 'flex', alignItems: 'center', gap: 3 }}>
      {Array.from({ length: max }, (_, i) => (
        <Dot key={i} filled={i < r} size={size} emptyColor="rgba(255,245,230,.9)" />
      ))}
    </span>
  )
}
